    ...
    <div className="pdfProPage">
      <section className="pdfProHero">
        ...
        <div className="pdfUploadArea">
          <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 220px",gap:12,alignItems:"stretch"}}>
            <div>
...
function BackgroundRemoverTool({t,back}) {
  ...
  const extraBackgrounds = Array.from({length:60},(_,i)=>`https://picsum.photos/seed/toolmaster-bg-${i+1}/900/900`);
  const allPhotoBackgrounds = [...photoBackgrounds, ...extraBackgrounds];
  ...
  const fastRemove = async sourceFile => {
    ...
    for(let y=0;y<h;y++){for(let x=0;x<w;x++){const i=(y*w+x)*4;const r=d[i],g=d[i+1],b=d[i+2];const dist=Math.hypot(r-avg[0],g-avg[1],b-avg[2]);const bright=(r+g+b)/3;const lowSat=Math.max(r,g,b)-Math.min(r,g,b)<30;if((dist<52 && (bright>160 || lowSat)) || (bright>238 && dist<82)) d[i+3]=0;}}
  ...
  const doCutout = async (sourceFile=file, useAI=false) => {
    ...
    setBusy(true);
    setStatus(useAI?"AI HD background removal…":"Fast background removal…");
    try{
      let blob;
      if(useAI){
  ...
  return <Shell back={back} t={t} status={status}>
    <div className="bgRemoveShell" style={{border:"1px solid #e4e6ed",borderRadius:18,background:"#fff",boxShadow:"var(--shadow)",overflow:"hidden"}}><div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",borderBottom:"1px solid #eceef4",background:"#fff",position:"sticky",top:0,zIndex:5}}><button type="button" className="btn" onClick={back}>← Back to Tools</button><button type="button" className="btn" onClick={()=>window.scrollTo({top:0,behavior:"smooth"})}>Tools</button><button type="button" className="btn" onClick={()=>{const x=tools.find(t=>t[3]==="seo-genius"); if(x) window.dispatchEvent(new CustomEvent("tm-open-tool",{detail:x}));}}>SEO Genius AI</button><div style={{marginLeft:"auto",fontSize:12,fontWeight:800,color:"#667085"}}>Image Editor</div></div>
      {!file ? uploadHero : <>
        <input ref={inputRef} type="file" accept="image/png,image/jpeg,image/webp" onChange={upload} style={{display:"none"}}/>
        ...
        <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 330px",minHeight:560}}>
          <div style={{...checkerStyle,backgroundColor:"#fff",display:"grid",placeItems:"center",padding:26}}>
            <div style={{width:"min(760px,100%)",height:"min(590px,72vh)",borderRadius:18,overflow:"hidden",display:"grid",placeItems:"center",position:"relative",border:"1px solid #dfe4eb",boxShadow:"0 12px 40px rgba(15,23,42,.08)",background:bgImage?`url("${bgImage}") center/cover no-repeat`:bgMode==="white"?"#fff":bgMode==="black"?"#000":bgMode==="color"?bgColor:"transparent"}}>
              ...
              {busy && <div style={{position:"absolute",inset:0,display:"grid",placeItems:"center",background:"rgba(255,255,255,.58)",backdropFilter:"blur(2px)"}}><div style={{background:"#fff",padding:"12px 18px",borderRadius:999,fontWeight:700,boxShadow:"0 10px 30px rgba(15,23,42,.12)"}}>Processing…</div></div>}
            </div>
          </div>
          ...
          <aside style={{borderLeft:"1px solid #eceef4",background:"#fff",padding:18,overflow:"auto"}}>
            ...
            {tab==="background" && <div>
              ...
              {bgCategory==="photo" && <>
                ...
                <p className="bgCount">110+ ready-made wallpapers · or upload your own</p>
                <div className="bgGallery">
                  {allPhotoBackgrounds.map((src,i)=><button key={`${src}-${i}`} type="button" className={bgImage===src?"selected":""} onClick={()=>chooseBackground("image",src)}><img loading="lazy" src={src} alt={`Background ${i+1}`}/></button>)}
                </div>
